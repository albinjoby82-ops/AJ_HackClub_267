...
Everything below is current for the working tree based on commit `1f9c79a` on
branch `micromouse-26-reveal`.
...
- **`makerlabs_audience.jpeg` is not an event photo.** The supplied file is a
  generic hills-and-clouds placeholder, so it is deliberately excluded from
  `ASSETS` and the montage revisits the real hands-on image instead.
- **The previously omitted supplied visuals are now in the cut.**
  `roboexpo_demoers.png` and `roboexpo_teams.png` flash before the project run;
  the sand machine match-cuts between both supplied CAD views.
...
2. **Event details.** `[DATE]`, `[TIME]`, `[VENUE]`, and `[SIGN-UP URL]` in
   `scene08Title.js`.
   Do not invent these — `docs/07_EVENT_DETAILS_PLACEHOLDERS.md` is explicit.
3. **Self-host a condensed font.** Currently falls back through
...
The original deterministic soundtrack is generated by
`tools/generate-audio.mjs`; `npm run render` rebuilds it and muxes it as AAC.
