...
Start with USB and the onboard LED. Add the IMU, one distance sensor at a time, then the motors. Each page has the supplied sketch, its original wiring comments, expected output and a download. Fix a failing step before adding the next part.

<a href="../../../../downloads/micromouse-debug-kit.zip" download>Download the complete debug kit ZIP</a> · [Start step 1](${route}01_led_red.md) · [Event wiring map](#/docs/Micromouse2026/Hardware/ESP32C6.md)
...
6. Extract the ZIP and open the chosen sketch’s <code>.ino</code> inside its matching folder. Upload, open Serial Monitor, then press RESET to see startup messages.
...
| Old behavior after downloading a sketch | Open or paste the intended code in the active Arduino tab, then upload it. Downloading alone does not flash the board. |
...
The downloadable sketches and visualizer are copied unchanged from the event’s <code>micromouse-debug-kit</code>. Wiring and expected results come from their top-of-file comments. [Read the supplied README](${download}README.md). Each page includes those original comments and the complete sketch, so you can compare the instructions directly with what you upload.
`;
